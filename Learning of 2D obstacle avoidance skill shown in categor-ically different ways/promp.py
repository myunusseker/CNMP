...
class ProMP(object):
    ...
    """
    def __init__(self, nrBasis=31, sigma=0.05, num_samples=200):
    ...
    def generate_trajectory(self, randomness=1e-10):
        ...
        for viapoint in self.viapoints:
            PhiT = np.exp(-.5 * (np.array(map(lambda x: x - self.C, np.tile(viapoint['t'], (self.nrBasis, 1)).T)).T ** 2 / (self.sigma ** 2)))
	...
	#sampW = np.random.multivariate_normal(newMu, randomness*newSigma, 1).T
        return np.dot(self.Phi.T, newMu) , np.sqrt(np.diag(np.dot(self.Phi.T, np.dot(newSigma, self.Phi))))

    def plot(self, x=None, legend='promp', color=None):
        #print 'Cs = ',self.C
        #print 'Phi Shape ',self.Phi.shape,'Phi = ',self.Phi
        #print 'W Shape ',self.W.shape,'W = ',self.W
        #print 'meanW Shape ',self.meanW.shape,'meanW = ',self.meanW
        mean = np.dot(self.Phi.T, self.meanW)
        x = self.x if x is None else x
	#plt.plot(x, mean, color=color, label=legend)
        std = np.sqrt(np.diag(np.dot(self.Phi.T, np.dot(self.sigmaW, self.Phi))))
